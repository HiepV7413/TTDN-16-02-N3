...
class NhanVien(models.Model):
    ...
    trang_thai_hop_dong = fields.Selection(
        related='hop_dong_hien_tai_id.trang_thai',
        string='Trạng thái hợp đồng',
        readonly=True
    )
    
    @api.depends('danh_sach_hop_dong_ids', 'danh_sach_hop_dong_ids.trang_thai')
    def _compute_hop_dong_hien_tai(self):
        for record in self:
            hop_dong = record.danh_sach_hop_dong_ids.filtered(
